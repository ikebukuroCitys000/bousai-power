...
# サイトのCSSアクセントカラー（assets/css/style.scssの--accent系）に寄せつつ、
# ポップで可愛いレトロ漫画タッチに変更（2026-08-02 ユーザー指定のリファレンス
# 方向性を反映：太い黒の輪郭線＋ハーフトーン網点陰影＋元気なキャラクター）。
IMAGE_STYLE_ANCHOR = (
    "ポップで可愛いレトロ漫画・ステッカー風のイラストタッチ。"
    "太めの黒い輪郭線でくっきりと縁取りし、影の部分にハーフトーン（網点）の"
    "ドットパターンを使う。背景は明るいクリーム色〜白を基調にし、"
    "メインカラーにオレンジ/アンバー（#f5c451〜#f2994aあたり）、"
    "差し色にグリーン（#4fd1a5あたり）を使った、コントラストの効いたポップな配色。"
    "人物やキャラクターを描く場合は、丸みを帯びた可愛らしいデフォルメ、"
    "生き生きとした動きのあるポーズにする。厚塗りやフォトリアルな質感は避け、"
    "シンプルな面・輪郭線・ハーフトーンだけで構成する、シリーズ7枚に共通するトーン＆マナー。"
)
IMAGE_NEGATIVE_INSTRUCTION = (
    "画像内に文字・数字・単位・吹き出し・ラベル・ロゴ・透かし・サイン（作者名風の手書き文字）は"
    "絶対に描画しないこと（テキストはCanva側で別レイヤーとして重ねるため、"
    "画像に文字が写り込むと二重表示になり不可）。"
    "実在する人物・有名人・既存ブランドのロゴやマスコットキャラクターも描かないこと。"
)
...
- 各スライドの image_prompt には、その内容を視覚的に補うイラスト・写真の指示を1文で書く（例:「停電した部屋でスマホの充電が切れて困っている人のフラットイラスト」）。実在の人物・商標ロゴを指定しない。Canvaの画像生成（マジック生成）にそのまま使える具体性を持たせる。**数式・計算式・数値ラベル・グラフの目盛りなど「文字や数字そのものが主役になる被写体」は指定しない**（画像生成AIが文字を描き込んでしまうため）。数値の対比を表現したい場合も、具体物の大きさ・量・個数の違いなど、文字を使わない絵で表現できる構図にする（例:「1×3=2」ではなく「小さい四角3つ分の大きさの箱」のように）
